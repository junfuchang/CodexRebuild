...
它的重点是让使用 API key 的 Windows 版 Codex 也能开启 Fast mode、显示 Plugins 入口，并补上本地会话永久删除能力。更新 Codex 核心只是附带维护功能，可以按需使用。
...
- 你使用 API key 登录 Codex，但仍希望使用 Fast mode 和 Plugins 入口。

## 亮点

- 为使用 API key 的 Windows 版 Codex 开启 Fast mode。
- 让 API key 登录状态也能显示 Plugins 入口。
- 增加真实的本地会话永久删除按钮：左侧会话列表可以右键删除，设置里的已归档对话也可以删除。
- 删除会话时会清理本地会话记录及相关文件，并提示重启 Codex 刷新会话列表。
- 所有补丁都作用于可写副本 `.\Codex`，原始 Microsoft Store 应用保持不变。
...
这不是主要亮点。只有在你已经下载新版本核心，或需要替换 `codex.exe`、`codex-windows-sandbox.exe`、`codex-windows-sandbox-setup.exe` 时再使用。
